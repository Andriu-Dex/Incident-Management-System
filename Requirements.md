...
**Arquitectura:**
- **Onion Architecture** (capas separadas: dominio, aplicación, infraestructura, presentación)

**Backend:**
- **Framework:** .NET 8 (Core) con C#
- **Base de datos:** SQL Server
- **ORM:** Entity Framework Core

**Frontend:**
- **Framework UI:** Blazor Server
  - Justificación: Más simple para desarrollo, ideal para aplicaciones internas, menor curva de aprendizaje
  - SignalR integrado para notificaciones en tiempo real
  - Acceso directo a recursos del servidor

**Librerías y Frameworks Backend:**

1. **MediatR** (Obligatorio)
   - Implementación de patrón CQRS
   - Desacoplamiento de comandos y queries
   - Facilita testing y mantenibilidad

2. **FluentValidation** (Obligatorio)
   - Validación de comandos y queries
   - Código más limpio que Data Annotations
   - Validaciones complejas y reutilizables

3. **AutoMapper** (Recomendado)
   - Mapeo entre entidades y DTOs
   - Reduce código boilerplate

4. **ASP.NET Core Identity** (Obligatorio)
...
1. **Tailwind CSS** (Principal - 80%)
   - Framework CSS utilitario
   - Desarrollo rápido y consistente
   - Sistema de diseño predefinido
   - Responsive design simplificado
   - Ayuda a cumplir requisitos de usabilidad (ISO 9241)

2. **Scoped CSS** (Complementario - 20%)
   - Para componentes específicos
   - Animaciones complejas
   - Casos donde Tailwind sea verboso
...
5. **Blazored.LocalStorage** (Opcional)
   - Almacenamiento local del navegador
   - Preferencias de usuario

6. **ChartJs.Blazor** (Para Fase 8)
   - Gráficos y estadísticas
   - Dashboard de administración

**Requisitos Mínimos de Implementación:**

- Separar **capas** (Domain, Application, Infrastructure, Web/UI)
...
│  • Scoped CSS (casos específicos)           │
│  • Blazored.Toast (notificaciones)          │
│  • Blazored.Modal (modales)                 │
│  • Razor Components                          │
└─────────────────────────────────────────────┘
                      │
                      │ SignalR / HTTP
...
│  • Entities (User, Incident, Service, etc.) │
...
### Flujo de Datos:
1. **Usuario** → interactúa con **Blazor Components** (Presentation)
2. **Components** → envían **Commands/Queries** a través de **MediatR** (Application)
3. **Handlers** → ejecutan lógica de negocio y acceden a **Repositories** (Application)
4. **Repositories** → interactúan con **EF Core** para acceso a datos (Infrastructure)
5. **EF Core** → ejecuta operaciones en **SQL Server** (Infrastructure)
6. **Response** → regresa por las capas hasta el componente Blazor
...
- ✅ **Single Responsibility**: Cada clase/handler tiene una única responsabilidad
- ✅ **Open/Closed**: Extensible mediante nuevos handlers sin modificar código existente
