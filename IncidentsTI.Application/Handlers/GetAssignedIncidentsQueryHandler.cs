...
public class GetAssignedIncidentsQueryHandler : IRequestHandler<GetAssignedIncidentsQuery, IEnumerable<IncidentListDto>>
{
    ...
    public async Task<IEnumerable<IncidentListDto>> Handle(GetAssignedIncidentsQuery request, CancellationToken cancellationToken)
    {
        var incidents = await _incidentRepository.GetByAssignedToIdAsync(request.AssignedToId);
